...
function EstadisticasPersonales() {
  ...
  const horasTotales = juegos.reduce(
    (acc, j) => acc + (Number(j.horasJugadas) || 0),
    0
  )
  const horasPromedio = totalJuegos ? horasTotales / totalJuegos : 0

  const puntuacionMedia = totalJuegos
    ? juegos.reduce((acc, j) => acc + (Number(j.puntuacion) || 0), 0) /
      totalJuegos
    : 0

  // Últimos juegos agregados (más recientes primero)
  const juegosRecientes = [...juegos]
    .sort(
      (a, b) =>
        new Date(b.fechaAgregado || b.createdAt || 0) -
        new Date(a.fechaAgregado || a.createdAt || 0)
    )
    .slice(0, 5)

  // Juegos por plataforma (cantidad)
  ...
  juegos.forEach(j => {
    ...
    plataformas.forEach(p => {
      if (!juegosPorPlataformaMap[p]) juegosPorPlataformaMap[p] = 0
      juegosPorPlataformaMap[p] += 1
    })
  })
  ...
  // Distribución de puntuaciones por rangos (1.0–10.0)
  const rangos = [
  ...
  const distribucionPuntuacion = rangos.map(rango => {
    const cantidad = juegos.filter(j => {
      const p = Number(j.puntuacion) || 0
      return p >= rango.min && p < rango.max
    }).length
    return { ...rango, cantidad }
  })

  const maxBarDistribucion = Math.max(
    ...distribucionPuntuacion.map(r => r.cantidad),
    1
  )
  const maxBarPlataformas = Math.max(
    ...juegosPorPlataforma.map(p => p.cantidad),
    1
  )
  const maxBarGeneros = Math.max(
    ...juegosPorGenero.map(g => g.cantidad),
    1
  )
  ...
  {/* IZQUIERDA: stats + juegos recientes */}
    ...
    <div className="estadisticas-grid">
      <div className="stat-card acrylic-card">
        ...
        <div className="stat-extra">
          Todos los juegos de tu biblioteca
        </div>
      </div>
          ...
          <div key={juego._id} className="item-reciente">
            ...
            <div className="info-reciente">
              ...
              <p>
                ...
                {(Number(juego.puntuacion) || 0).toFixed(1)}/10
              </p>
              {juego.completado && (
          ...
          <div key={rango.label} className="chart-bar-row">
            ...
            <div className="chart-bar-track">
              <div
                ...
                style={{
                  width: `${
                    (rango.cantidad / maxBarDistribucion) * 100
                  }%`
                }}
              />
            </div>
          ...
          <div key={p.nombre} className="chart-bar-row">
            ...
            <div className="chart-bar-track">
              <div
                ...
                style={{
                  width: `${
                    (p.cantidad / maxBarPlataformas) * 100
                  }%`
                }}
              />
            </div>
